...
def scan_image_for_barcode(img_bgr: np.ndarray) -> Optional[dict]:
    """
    Escanea una imagen BGR en escala de grises y en 4 rotaciones (0, 180, 90, 270)
    buscando códigos de barras PDF417 de cédulas colombianas.
    """
    if img_bgr is None or img_bgr.size == 0:
        return None

    # Probar primero en tamaño natural y luego con ligero reescalado si la imagen es pequeña
    h, w = img_bgr.shape[:2]
    scales = [1.0]
    if max(h, w) < 800:
        scales.append(1.5)
    ...
    for scale in scales:
        ...
        # Priorizar 0° y 180° que abarcan el 99% de orientaciones
        rotaciones = [
            (None, "0_grados"),
        ...
        for rot_code, label in rotaciones:
            ...
            try:
                barcodes = zxingcpp.read_barcodes(scan_img)
                for b in barcodes:
                    if b.format == zxingcpp.BarcodeFormat.PDF417:
