...
def extract_text_from_id(image: np.ndarray) -> Optional[Dict[str, Any]]:
    ...
    try:
        # Redimensionar a una escala óptima para inferencia rápida en CPU (ancho max 1280px)
        h, w = image.shape[:2]
        max_dim = 1280
        if max(h, w) > max_dim:
...
def parse_id_card_lines(lines: List[str]) -> Optional[Dict[str, Any]]:
    ...
    nombres = ""
    doc_index = -1
    es_contrasena = 'CONTRASENA' in full_text or 'CONTRASEÑA' in full_text or 'COMPROBANTE' in full_text

    # 1. Buscar NUIP o Número con puntos (ej: 1.117.509.911 o 1.117.513.499 o 1117.513.499)
    for i, line in enumerate(lines):
        l_u = line.strip().upper()

        # Formato con prefijo NUMERO / NUIP / PRIMERAVEZCC (ej: NUIP1.117.509.911 o NUMERO 1.116.914.645)
        ...
        # Formato con puntos exacto sin prefijo (ej: 1.117.509.911 o 1117.513.499)
    ...
    # 2. Buscar patrón MRZ (Cédula Digital) si aún no tiene número o para confirmar
    # Ej: '8606073M3308165C0L1127070363<5'
    if not doc_number:
        for line in lines:
    ...
    # 3. Buscar número al pie del código de barras (ej: 'P-4400300-67115951-M-0017711201-20030702')
    if not doc_number:
        m_bar_foot = re.search(r'-[MF]-0*(\d{6,10})-', full_text)
        if m_bar_foot:
            doc_number = m_bar_foot.group(1)

    # 4. Fallback de búsqueda de dígitos sueltos de 7 a 10 cifras (evitando códigos de comprobante de contraseña)
    if not doc_number:
        for i, line in enumerate(lines):
            m_any = re.search(r'\b(1\d{8,9}|\d{7,8})\b', line)
            if m_any and not line.startswith('851'): # evitar codigo de barras de contraseña
    ...
    for i, line in enumerate(lines):
        ...
        # 1. Detectar etiqueta combinada en Contraseñas Registraduría (ej: 'ARELIEOSNOMIE', 'APELLIDOS Y NOMBRES', 'APELLIDOS NOMBRES')
        ...
        # 3. Detectar variantes de etiqueta NOMBRES (ej. NOMBRES, NOMORES, NDMOAES, NBMOAES, NOMBR)
