...
document.addEventListener('DOMContentLoaded', () => {
    ...
    uploadForm?.addEventListener('submit', (e) => {
        ...
        // Cronómetro en tiempo real (minutos:segundos.décimas)
        ...
        const timerInterval = setInterval(() => {
            ...
            if (timerDisplay) {
                timerDisplay.innerText = `${min}:${sec}.${tenths}`;
            }
        }, 100);
    ...
    // Manejador para eliminar ficha desde la tabla principal del dashboard
    document.addEventListener('click', async (e) => {
        const btn = e.target.closest('.btn-eliminar-ficha');
        if (!btn) return;

        e.preventDefault();
        const fichaId = btn.getAttribute('data-ficha-id');
        const codigo = btn.getAttribute('data-codigo');

        if (!confirm(`¿Estás seguro de eliminar la Ficha N° ${codigo}? Se borrarán los datos asociados para permitir una nueva carga limpia.`)) {
            return;
        }
        ...
        try {
            ...
            params.append('ficha_id', fichaId);
            if (csrfToken) {
