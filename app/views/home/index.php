                        ...
                        <tbody>
                            <?php foreach ($fichas as $f): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($f['tiempo_procesamiento_seg'])): ?>
                                            <?php 
                                            $seg = (float)$f['tiempo_procesamiento_seg'];
                                            $minStr = floor($seg / 60) > 0 ? floor($seg / 60) . 'm ' : '';
                                            $secStr = round(fmod($seg, 60), 1) . 's';
                                            ?>
                                            <span style="display: inline-flex; align-items: center; gap: 4px; font-weight: 600; color: #4338ca; background: #e0e7ff; padding: 3px 8px; border-radius: 9999px; font-size: 0.82rem;">
                                                ⏱ <?= $minStr . $secStr ?>
                                            </span>
                                        <?php else: ?>
                                    ...
                                    <td style="white-space: nowrap; width: 220px;">
                                        <div class="action-buttons" style="display: flex; gap: 8px;">
                                            ...
                                            <button type="button" class="btn-sm btn-danger btn-eliminar-ficha" data-ficha-id="<?= (int)$f['id'] ?>" data-codigo="<?= htmlspecialchars($f['codigo_ficha'], ENT_QUOTES, 'UTF-8') ?>" title="Eliminar Ficha">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
